...
const Message: React.FunctionComponent<IMessageProps> = (props) => {
    const [messageContent, setMessageContent] = React.useState<JSX.Element | string | null>(null);

    React.useEffect(() => {
        let text: JSX.Element | string = props.text;

        // if text is a image link, transform text to img tag
        if(isImageLink(text)) {
            text = <ImageMessage src={text} />
        }
